        ...
        {activeChat ? (
          <>
            <div className="bg-white border-b shadow-sm z-30">
              <div className="px-6 py-3 border-b flex items-center justify-between relative">
                ...
                <div className="relative">
                  <button onClick={(e) => { e.stopPropagation(); setShowMenu(!showMenu); }} className="p-2 hover:bg-gray-100 rounded-full transition-colors relative z-40">
                  ...
                  {showMenu && (
                    <>
                      ...
                      <div className="fixed right-4 md:right-8 mt-2 w-52 bg-white border border-gray-200 rounded-2xl shadow-2xl py-2 z-[101] overflow-hidden" style={{ top: '55px' }}>
                        <button onClick={handleLeaveChat} className="w-full px-4 py-3 text-left text-sm font-bold text-gray-600 hover:bg-gray-50 flex items-center gap-3"><LogOut size={16} /> Leave Chat</button>
                        <button onClick={handleBlockChat} className="w-full px-4 py-3 text-left text-sm font-bold text-red-600 hover:bg-red-50 flex items-center gap-3 border-t border-gray-100"><Ban size={16} /> Block User</button>
                      </div>
                    </>
                  )}
                </div>
                ...
                <div className="px-6 py-4 flex items-center justify-between bg-gray-50/50">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-black text-green-700 leading-none">Rs. {activeAd.price?.toLocaleString()}</p>
                    </div>
                  </div>
                  <Link to={`/ad/${activeChat.adId}`} className="bg-green-700 text-white px-4 py-2.5 rounded-xl text-[10px] font-black flex items-center gap-2">VIEW FULL AD <ExternalLink size={12} /></Link>
                </div>
              )}
            </div>
                ...
                <div key={msg.id} className={`flex ${msg.senderId === user.uid ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-3 rounded-2xl ${msg.senderId === user.uid ? 'bg-green-700 text-white rounded-tr-none' : 'bg-white text-gray-800 rounded-tl-none border shadow-sm'}`}>
                    ...
                    <div className="flex items-center justify-end gap-1 mt-1 opacity-70">
                      <span className="text-[9px] font-bold">{msg.timestamp?.toDate().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      {msg.senderId === user.uid && (msg.status === 'seen' ? <CheckCheck size={14} className="text-blue-300" /> : <Check size={14} />)}
                    </div>
                  </div>
                </div>
              ))}
            ...
            {activeChat.status === 'blocked' ? (
              <div className="p-6 bg-red-50 text-red-700 border-t flex items-center justify-center gap-3 italic text-xs font-bold uppercase"><ShieldAlert size={18} /> Chat is blocked</div>
            ) : (
